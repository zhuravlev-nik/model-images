...
const defaultOptions = {
  ...
  sizes: {
    detailSmall: [50, 50],
    detailNormal: [300, 300],
    detailLarge: [600, 600, 100],
    list: [150, 150]
  }
};
...
function ModelImagesPlugin(Schema, options) {
  ...
  Schema.pre('save', function(next) {
    ...
    if (options.fields instanceof Array) {
      options.fields.forEach(function(field) {
        if (!curObject.isModified(field)) return;
        imagesModified = true;
            ...
            if (mongoose.Types.ObjectId.isValid(img)) {
              ...
              async.series([
                  ...
                  function(cbFile) {
                    ...
                    let is = fs.createReadStream(img);
                    let os = fs.createWriteStream(sourcePath);
                    is.pipe(os);
                    is.on('end', function() {
                      fs.unlink(img, cbFile);
                      curImg.source = sourceUrl;
                      curImg.sourcePath = sourcePath;
                      curImg.ext = fileExt;
                    });
                  },
                  function(cbFile) {
                    let sizes = options.sizes;
                    let sourceFile = curImg.sourcePath;
                    curImg.sizes = {};
                    async.eachOf(sizes, function(size, alias, _cb) {
                      let curImgName = alias + curImg.ext;
                      let curImgPath = imgObjectPath + '/' + curImgName;
                      let curImgUrl = imgObjectUrl + '/' + curImgName;
                      gm(sourceFile).resize(size[0], size[1]).write(curImgPath, function(err) {
                        if (err) return _cb(err);
                        curImg.sizes[alias] = curImgUrl;
                        return _cb();
                      });
                    }, function(err) {
                      if (err) return cbFile(err);
